...
# ============================================================
# Ensemble infer: EfficientNetV2-S + ConvNeXt-Tiny + EfficientNetV2-M
# Weight: 0.3 * EfficientNetV2-S + 0.4 * ConvNeXt-Tiny + 0.3 * EfficientNetV2-M
# Input: X is a BGR image read by cv2 / platform input
...
# 三个权重文件名：按你说的命名
ENET_S_CKPT_NAME = "best_f1_enetv2_s.pth"
CV_CKPT_NAME = "best_f1_cv.pth"
ENET_M_CKPT_NAME = "best_f1_enetv2_m.pth"

# 如果平台要求文件放在 results 或 code_mo，也能自动找；同时兼容你本地训练输出目录
ENET_S_CANDIDATE_PATHS = [
    os.path.join(BASE_DIR, ENET_S_CKPT_NAME),
    os.path.join(BASE_DIR, "results", ENET_S_CKPT_NAME),
    os.path.join(BASE_DIR, "code_mo", ENET_S_CKPT_NAME),
    os.path.join("/home/wdy/cyp/weather/enetv2_s_ms_tta_v2", ENET_S_CKPT_NAME),
    os.path.join("/home/wdy/cyp/weather/enetv2_s_ms_tta_v2", "best_f1.pth"),
    os.path.join("/home/cyp/speedsci/weather/enetv2_s_ms_tta_v2", ENET_S_CKPT_NAME),
...
ENET_M_CANDIDATE_PATHS = [
    os.path.join(BASE_DIR, ENET_M_CKPT_NAME),
    os.path.join(BASE_DIR, "results", ENET_M_CKPT_NAME),
    os.path.join(BASE_DIR, "code_mo", ENET_M_CKPT_NAME),
    os.path.join("/home/wdy/cyp/weather/enetv2_m_ms_tta_v1", ENET_M_CKPT_NAME),
    os.path.join("/home/wdy/cyp/weather/enetv2_m_ms_tta_v1", "best_f1.pth"),
    os.path.join("/home/cyp/speedsci/weather/enetv2_m_ms_tta_v1", ENET_M_CKPT_NAME),
    os.path.join("/home/cyp/speedsci/weather/enetv2_m_ms_tta_v1", "best_f1.pth"),
]
...
def _build_enetv2_m(num_classes=4, dropout=0.40):
...
def _load_enet_s_model():
    ckpt, _ = _load_checkpoint(ENET_S_CANDIDATE_PATHS, "EfficientNetV2-S")
...
def _load_enet_m_model():
    ckpt, _ = _load_checkpoint(ENET_M_CANDIDATE_PATHS, "EfficientNetV2-M")
    state_dict, labels, args = _extract_ckpt(ckpt)
    dropout = float(args.get("dropout", 0.40))

    model = _build_enetv2_m(num_classes=len(labels), dropout=dropout).to(DEVICE)
    model.load_state_dict(state_dict, strict=True)
    model.eval()
    return model, labels
...
enet_s_model, enet_s_labels = _load_enet_s_model()
cv_model, cv_labels = _load_cv_model()
enet_m_model, enet_m_labels = _load_enet_m_model()

# 输出标签顺序优先使用 enetv2_s 的 class_to_idx；正常情况下都是 cloudy/rainy/snowy/sunny
label = enet_s_labels

if set(enet_s_labels) != set(cv_labels):
    raise RuntimeError(f"Label mismatch: enet_s={enet_s_labels}, convnext={cv_labels}")
if set(enet_s_labels) != set(enet_m_labels):
    raise RuntimeError(f"Label mismatch: enet_s={enet_s_labels}, enet_m={enet_m_labels}")
...
def predict(X):
    ...
    """
    enet_s_prob = _predict_prob_single_model(enet_s_model, X)
    cv_prob = _predict_prob_single_model(cv_model, X)
    enet_m_prob = _predict_prob_single_model(enet_m_model, X)

    enet_s_prob = _align_prob(enet_s_prob, enet_s_labels, label)
    cv_prob = _align_prob(cv_prob, cv_labels, label)
    enet_m_prob = _align_prob(enet_m_prob, enet_m_labels, label)

    final_prob = (
        ENET_S_WEIGHT * enet_s_prob
        + CV_WEIGHT * cv_prob
        + ENET_M_WEIGHT * enet_m_prob
    )
